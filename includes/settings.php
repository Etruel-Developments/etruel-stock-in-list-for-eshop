    ...
    */
    public static function settings_tab( $tabs ) {
        ...
        if (!isset( $tabs['extensions']['boilerplate'])) {
           	$tabs['extensions']['boilerplate'] = array('text' => __( 'Boilerplate', FKTR_STOCK_PRODUCT_TEXT_DOMAIN ), 'url' => admin_url('admin.php?page=fktr-stock-product-extension-page'), 'screen' => 'admin_page_fktr-stock-product-extension-page');
        }
        ...
        if (empty($tabs['extensions']['default']['screen']) && empty($tabs['extensions']['default']['url'])) {
           	$tabs['extensions']['default'] = array('text' => __( 'Extensions', FAKTURO_TEXT_DOMAIN ), 'url' => admin_url('admin.php?page=fktr-stock-product-extension-page'), 'screen' => 'admin_page_fktr-stock-product-extension-page');
        }
        return $tabs;
    }	
	...
	public static function page() {
		...
		echo '<div id="tab_container">
			<br/>
			<h1>'.__( 'Boilerplate Settings', FKTR_STOCK_PRODUCT_TEXT_DOMAIN ).'</h1>
			<form action="'.admin_url( 'admin-post.php' ).'" id="form_boilerplate" method="post">
				...
				echo '<table class="form-table">
						<tr valign="top">
							<th scope="row">'.__( 'Field', FKTR_STOCK_PRODUCT_TEXT_DOMAIN ).'</th>
							<td>
								<input type="text" name="fktr_boilerplate[field]" id="fktr_boilerplate_field" value="'.$values['field'].'"/>
								<p class="description">'.__( 'Description of field', FKTR_STOCK_PRODUCT_TEXT_DOMAIN ).'</p>
							</td>
						</tr>
					</table>';
				submit_button();
	...
	public static function save() {
		if ( ! wp_verify_nonce($_POST['_wpnonce'], 'save_boilerplate' ) ) {
		    wp_die(__( 'Security check', FKTR_STOCK_PRODUCT_TEXT_DOMAIN )); 
		}
		update_option('fktr_boilerplate_settings', $_POST['fktr_boilerplate']);
		fktrNotices::add(__( 'Settings updated', FKTR_STOCK_PRODUCT_TEXT_DOMAIN ));
