	...
	class fkrt_stock_in_list_proccess {
		...
		public static function manage_columns($column, $post_id) {
			...
			switch ($column) {
				case 'stock':
					...
					if ($total_stock < 0) {
						echo '<span style="color:' . $data_color_stock['out_stock'] . ';">'. __('Sold out', 'etruel-stock-in-list-for-eshop'). ' (' . $total_stock . ') </span>';
					} else if ($total_stock == 0) {
						echo '<span style="color:' . $data_color_stock['out_stock'] . ';">'. __('Sold out', 'etruel-stock-in-list-for-eshop'). '</span>';
					} else if ($total_stock >= 1 && $total_stock <= 5) {
						echo '<span style="color:' . $data_color_stock['low_stock'] . ';">'. __('Low stock', 'etruel-stock-in-list-for-eshop'). ' (' . $total_stock . ') </span>';
					} else {
						echo '<span style="color:' . $data_color_stock['in_stock'] . ';">'. __('In stock', 'etruel-stock-in-list-for-eshop'). ' (' . $total_stock . ') </span>';
					}
		...
		public static function print_buttons_reports($request) {
			...
			if ('edit.php' == $pagenow && $post_type == 'fktr_product') {
				...
				<a class="button-secondary right" title="Download CSV" href="<?php echo admin_url('admin-post.php?action=products_download_csv&' . http_build_query($request)) ?>" >
					<?php echo __('Download CSV', 'etruel-stock-in-list-for-eshop') ?>
				</a>
				<button class="button-secondary right btn_download_pdf" title="Download PDF" style="margin-right:10px;" >
					<?php echo __('Download PDF', 'etruel-stock-in-list-for-eshop') ?> 
				</button>
		...
		public static function fkrt_stock_in_list_download_csv() {
			...
			if (is_wp_error($currencyDefault)) {
				$total_html_print .= '<p>' . __('Account Movements needs the default currency on system settings.', 'etruel-stock-in-list-for-eshop') . '</p>';
		...
		public static function fkrt_stock_in_list_print_pdf() {
			...
			if (!empty($objects_client)) {
				...
				$html_objects = '<div style="clear: both;"><h2>' . __('No results with this filters', 'etruel-stock-in-list-for-eshop') . '</h2></div>';
			}
